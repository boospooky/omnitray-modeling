...
def calc_diffusion(y, diff_terms, p0):
    ...
    laplace_op_noflux_boundaries_onespec(Dc*dx*y[cs_i,:,:], diff_terms[cs_i,:,:])
    laplace_op_noflux_boundaries_onespec(Dc*dx*y[cp_i,:,:], diff_terms[cp_i,:,:])
    ...
    laplace_op_noflux_boundaries_onespec(Dc*dx*y[s_i,:,:], diff_terms[s_i,:,:])
    laplace_op_noflux_boundaries_onespec(Dc*dx*y[r_i,:,:], diff_terms[r_i,:,:])
    #diff_terms[s_i,:,:] = 0
    #diff_terms[r_i,:,:] = 0
...
def calc_rxn(y, d_y, nut_avail, p0):
    ...
    # AHL production
    d_y[a_i,:,:] =  scale*xa * y[s_i,:,:]*(y[cp_i,:,:]+y[cs_i,:,:]) - pa * y[a_i,:,:] - dx * Da * leak * y[a_i,:,:]
...
class Jacobian(object):
    ...
    def calc_dif_jac(self):
        ...
        for D_val, spec_ind in zip(D_vec, np.arange(species)):
...
#         dcdcdt_indices, dcdndt_indices, dndndt_indices, dndcdt_indices = self.rxn_indices_list
        ...
        #da/(dadt)
        v, u = a_i, a_i
        val_arr = -(pa+dx*leak*Da)*np.ones_like(nut_avail)
        i = self.assign_rxn_vals(self.rxn_indices_dict[(v,u)], val_arr,i)
...
class Simulator(object):
    ...
    def __init__(self, scale=32):
        ...
        self.initial_array[cs_i,y,x] = 1
        for i in range(50):
          rad = np.random.beta(1,1.5,size=(1,))*0.7*self.scale
          theta = np.random.uniform(0,2*np.pi,size=(1,))
          x = np.int(nw/2 + np.cos(theta)*rad)
          y = np.int(nh/2 + np.sin(theta)*rad)
          self.initial_array[cp_i,y,x] = 1
