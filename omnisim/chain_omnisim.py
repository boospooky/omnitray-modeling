...
cell_inds = (cs_i, cra_i, crb_i, crc_i, crd_i, cp_i)
protein_inds = (sa_i, sb_i, sc_i, sd_i, ra_i, rb_i, ra_i, rb_i)
ahl_inds = (aa_i, ab_i, ac_i, ad_i)
syn_inds = (sa_i, sb_i, sc_i, sd_i)
rep_inds = (ra_i, rb_i, rc_i, rd_i)
ahl_prod_tuples = tuple(zip(ahl_inds, syn_inds, rep_inds, cell_inds[:4]))
p0_keys='dx','Dc','rc','rS','rR','Hn','Kn','Dn','kn','Da','xa','xs','xS','xr','hS','kS','hR','kR','hC','kC','pa','leak','od'
spec_keys = "cs_i", "cra_i", "crb_i", "crc_i", "crd_i", "cp_i", "n_i", "aa_i", "ab_i", "ac_i", "ad_i", "sa_i", "sb_i", "sc_i", "sd_i", "ra_i", "rb_i", "rc_i", "rd_i"
...
def calc_rxn(y, d_y, nut_avail, p0):
    ...
    for a_i, s_i, r_i, c_i in ahl_prod_tuples:
        d_y[a_i,:,:] =  scale * xa * y[s_i,:,:]*(y[cp_i,:,:]+y[c_i,:,:]) - pa * y[a_i,:,:] 
        d_y[r_i,:,:] = ( xr* hill(y[a_i,:,:], hR, kR)*np.greater(y[cp_i,:,:],od)  - rc * y[r_i,:,:]) * nut_avail - rR * y[r_i,:,:]

    # Synthase production
    a_i, s_i, r_i, c_i = ahl_prod_tuples[0]
    d_y[s_i,:,:] = ( xs * np.greater(y[cp_i,:,:],od) * hill(y[a_i,:,:], hS, kS) * hillN(y[r_i,:,:], hC, kC) + xS * np.greater(y[c_i,:,:],od) - rc * y[s_i,:,:]* np.greater(y[cp_i,:,:]+y[c_i,:,:],od)) * nut_avail - rS * y[s_i,:,:]
    ba_i = a_i
    for a_i, s_i, r_i, c_i in ahl_prod_tuples[1:]:
        d_y[s_i,:,:] = ( xs * np.greater(y[cp_i,:,:],od) * hill(y[a_i,:,:], hS, kS) * hillN(y[r_i,:,:], hC, kC) + xS * hill(y[ba_i,:,:], hS, kS)* np.greater(y[c_i,:,:],od) - rc * y[s_i,:,:]* np.greater(y[cp_i,:,:]+y[c_i,:,:],od)) * nut_avail - rS * y[s_i,:,:]
        ba_i = a_i
